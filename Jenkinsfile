                    ...
                    if (targetTomcat == 'tomcat-1') 
						{
							echo "Deploying ${params.BRANCH_NAME} branch to ${targetTomcat}..."
							 deploy adapters: [tomcat9(credentialsId: 'tomcat9', path: '', url: 'http://13.232.88.151:8081')], contextPath: '/master', war: '**/*.war'
						} 
					else if
						{
							echo "Deploying ${params.BRANCH_NAME} branch to ${targetTomcat}..."	
							deploy adapters: [tomcat9(credentialsId: 'tomcat9', path: '', url: 'http://13.232.88.151:8081')], contextPath: '/Dev', war: '**/*.war'
						}
					else if
						{
							echo "Deploying ${params.BRANCH_NAME} branch to ${targetTomcat}..."	
							deploy adapters: [tomcat9(credentialsId: 'tomcat9', path: '', url: 'http://13.232.88.151:8081')], contextPath: '/Test', war: '**/*.war'
						}	
					else {
