pipeline {
	...
	stage('Email'){
		...
		steps {
			mail bcc: '', body: "${env.PROJECT_NAME} - Build # ${env.BUILD_NUMBER} - ${env.BUILD_STATUS}!", subject: "${env.PROJECT_NAME} - Build # ${env.BUILD_NUMBER} - ${env.BUILD_STATUS}:Check console output at ${env.BUILD_URL} to view the results."', to:'shubhamalagur@gmail.com'}   
    }
}
}
